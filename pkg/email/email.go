...
//Send  send mail
func Send(subject string, body []byte, tos []string) error {
	config := `{
			"username": "mfsdk@morefuns.com.cn",
			"password": "morefun20082015",
			"host":     "smtp.exmail.qq.com",
			"port":     25}`

	mail := NewEMail(config)
	mail.To = tos
	//	mail.From = common.AdminMail
	mail.From = "mfsdk@morefuns.com.cn"
	mail.Subject = subject
	mail.Text = string(body)
	mail.Send()
